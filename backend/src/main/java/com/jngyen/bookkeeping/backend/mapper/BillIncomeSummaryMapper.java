...
public interface BillIncomeSummaryMapper {
    ...
    // 删除收入汇总
    // 用对齐后的时间查询单条收入
    List<BillIncomeSummaryPO> selectOneIncomeSummaryByTimeAndType(String userUuid, String categoryName , BillSummaryTimeType budgetTimeType, LocalDate startDate, LocalDate endDate,Boolean isIncome);
    // 按照时间和时间类型查找区间内的全部收入汇总,获取对应的年、月、周、日的收入汇总
    List<BillIncomeSummaryPO> selectAllIncomeSummaryByTimeAndType(String userUuid, String categoryName , BillSummaryTimeType budgetTimeType, LocalDate startDate, LocalDate endDate,Boolean isIncome);
